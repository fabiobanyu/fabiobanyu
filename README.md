<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=250&section=header&text=Fabio%20Banyu%20Cyto&fontSize=50&fontColor=ffffff&animation=twinkling&fontAlignY=35&desc=Data%20Science%20Undergrad%20%7C%20AI%20and%20Machine%20Learning%20Enthusiast&descFontSize=16&descAlignY=55" width="100%" />
</div>

<div align="center">
<a href="https://git.io/typing-svg">
<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=20&duration=3000&pause=1000&color=00D2FF&center=true&vCenter=true&width=700&lines=Exploring+AI+through+Machine+Learning+🤖;Building+with+Deep+Learning+and+Computer+Vision+👁️;Exploring+Audio+Intelligence+and+Speech+Processing+🎵;Turning+AI+Models+into+Practical+Applications+⚡" alt="Typing Animation" />
</a>
</div>

<br>

<div align="center">
<a href="https://www.linkedin.com/in/fabio-banyu-cyto-1318782ab/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
<a href="mailto:fabiocyto74@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
<a href="https://wa.me/6285769715375"><img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" /></a>
<a href="https://www.instagram.com/biyokcb/"><img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" /></a>
</div>

<br><br>

<table align="center" width="100%" border="0" style="border-collapse: collapse;">
<tr>

<td width="55%" valign="top">

<h2 align="center">👋 About Me</h2>

<p align="justify">
I am a <b>Data Science undergraduate</b> at <b>Institut Teknologi Sumatera (ITERA)</b> with a strong interest in <b>Artificial Intelligence, Machine Learning, and Deep Learning</b>.
</p>

<p align="justify">
My main interests are in applying AI to <b>unstructured data</b>, especially images and audio. I enjoy learning how models work, experimenting with different approaches, and developing them into practical solutions. My current exploration focuses on <b>Computer Vision, Deep Learning, Audio Intelligence, and Speech Processing</b>.
</p>

<p align="justify">
Besides model development, I am also interested in the <b>application and integration of AI</b>. I enjoy connecting models with APIs and simple web interfaces so that AI systems can be used more practically. I am continuously improving my skills in model development, deployment, and end-to-end AI application development.
</p>

<br>

<p><strong>🔥 Main Areas of Interest:</strong></p>

<ul>
<li>🤖 <b>Machine Learning & Deep Learning</b> — Model Development, CNNs, PyTorch</li>
<li>👁️ <b>Computer Vision</b> — Image Classification, Attention Mechanisms, Explainable AI</li>
<li>🎵 <b>Audio Intelligence</b> — Speech Processing & Audio Analysis</li>
<li>💻 <b>AI Application & Integration</b> — Flask, APIs & Web Integration</li>
</ul>

</td>

<!-- BAGIAN INI KITA KOMBINASIKAN TROPHIES & STREAK STATS -->
<td width="45%" valign="top" align="center">

<h2 align="center">🏆 GitHub Trophies</h2>

<br>

<a href="https://github.com/ryo-ma/github-profile-trophy">
<img src="https://github-profile-trophy.vercel.app/?username=fabiobanyu&theme=radical&no-frame=true&no-bg=true&margin-w=15&column=2" width="100%" />
</a>

<br><br><br>

<!-- Streak Stats untuk mengisi Blank Space -->
<img src="https://github-readme-streak-stats.herokuapp.com/?user=fabiobanyu&theme=radical&hide_border=true&background=0D1117" width="100%" />

</td>

</tr>
</table>

<br>

<h2 align="center">🛠️ Tech Stack</h2>

<div align="center">
<a href="https://skillicons.dev">
<img src="https://skillicons.dev/icons?i=py,r,js,html,css,pytorch,sklearn,tensorflow,git,vscode,flask,linux,postman&perline=13" />
</a>
</div>

<br><br>

<h2 align="center">🚀 Featured Projects</h2>

<table align="center" width="100%" border="0" style="border-collapse: collapse;">

<tr>

<td width="50%" valign="top" align="center">

<h3>🍃 Hoya Leaf Disease Detection</h3>

<p align="justify">
An end-to-end <b>Computer Vision</b> project for detecting diseases in Hoya plant leaves. The project explores <b>ResNet-50</b> and <b>MobileNetV3</b> models for image classification, with a <b>Knowledge Graph using Neo4j</b> to support treatment recommendations. The system is integrated with <b>Flask</b> for application deployment.
</p>

<a href="https://github.com/fabiobanyu/Hoya-Leaf-Disease-Detection">
<img src="https://img.shields.io/badge/View_Project-150E28?style=for-the-badge&logo=github&logoColor=white" />
</a>

<br><br>

</td>

<td width="50%" valign="top" align="center">

<h3>🚂 National Railway Forecasting</h3>

<p align="justify">
A <b>Time-Series Analysis</b> project using <b>R</b> to analyze national railway passenger traffic. The project explores historical passenger patterns, seasonal trends, and forecasting to understand potential future demand.
</p>

<a href="https://github.com/fabiobanyu/TUBES-ADW-Forecasting-Penumpang-KA">
<img src="https://img.shields.io/badge/View_Project-150E28?style=for-the-badge&logo=github&logoColor=white" />
</a>

<br><br>

</td>

</tr>

<tr>

<td width="50%" valign="top" align="center">

<h3>🚶♂️ Stochastic Mobility Analysis</h3>

<p align="justify">
A <b>Markov Chain</b> project developed in <b>R</b> to explore student mobility patterns based on survey data. The analysis examines transition probabilities, short-term movement patterns, and steady-state probabilities.
</p>

<a href="https://github.com/fabiobanyu/TUBES-PemStok-Mobilitas-Mahasiswa">
<img src="https://img.shields.io/badge/View_Project-150E28?style=for-the-badge&logo=github&logoColor=white" />
</a>

<br><br>

</td>

<td width="50%" valign="top" align="center">

<h3>📊 Data Visualization Dashboard</h3>

<p align="justify">
An interactive <b>data visualization</b> project focused on presenting datasets through clear and intuitive
